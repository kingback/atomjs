 ...
 * 自定义事件
 * event.js
 * @author ningzbruc@gmail.com huya.nzb@alibaba-inc.com https://github.com/kingback
 * @date 2013-11-12
 * @version 0.0.1
 */
...
;(function() {
    ...
    var ON = 'on',
        ...
        each = Array.prototype.forEach ? function(arr, fn) {
            return Array.prototype.each.call(arr, fn);
        } : function(arr, fn) {
            if (arr && arr.length) {
    ...
    EventTarget.prototype = {
        ...
        publish: function(type, options) {
            var allSubs = this._getEventSubs(type);
        ...
        fire: function(type, obj) {
            var facade = new EventFacade(this, type, obj);
        ...
        _getEventSubs: function(type, when) {
            var events = this._evtCfg.events,
                allSubs = events[type] || (event[type] = {});
            
            return when ? allSubs : when == DEF ? [allSubs[when]] : allSubs[when];
        },
        ...
        _bubbleToTargets: function(type, when, facade) {
            ...
            each(targets, function(target) {
                newFacade = mix({}, facade);
                newFacade.target = target;
                targets._fireEvent(type, when, newFacade);
            });
        }
    };
     ...
     * @param {Object} 传递参数
     */
    function EventFacade(target, type, obj) {
        this.type = type;
        this.stopped = 0;
        this.prevented = 0;
        this.target = target;
        this.currentTarget = target;
        this.prefix = target._evtCfg.prefix;
        mix(this, obj);
    }
